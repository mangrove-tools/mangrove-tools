...
- `GET /api/analytics/benchmarks`: reads curated benchmark rows for analytics tools.

Allowed event names are `tool_started`, `sample_data_used`, `calculation_completed`, `analytics_cta_clicked`, and `affiliate_clicked`.

Allowed event metadata keys are `affiliate_partner`, `cta_id`, `result_state`, `sample_id`, `source`, `step`, `surface`, and `version`. The route drops all other metadata keys, including raw calculator inputs such as budgets, revenue, pipeline values, emails, or notes.
